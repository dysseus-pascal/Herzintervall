#!/bin/sh
# Die Rechnung in hrv_math.c pruefen - auf der Zielarchitektur.
#
...
E="--emulator emery"
LOG=$HOME/herzintervall-selftest.log

sh "$SRC/tools/sync_herzintervall.sh" "$SRC" selftest || exit 1
...
sed -n '/== Quadratwurzel ==/,/SELBSTTEST FERTIG/p' "$LOG" | sed 's/^\[[^]]*\] [^ ]*> //'
echo "-------------------------------------------"
